...
import static helpers.Constants.ST_HIGH_BOUND_WIZARD_HP_PER;
import static helpers.Constants.ST_LOW_BOUND_WIZARD_HP_PER;
...
public final class Wizard extends Player {
    ...
    public void selectStrategy() {
        int currentHP = getHp();
        int maxHP = getMaxHP();
        if (currentHP > maxHP / ST_LOW_BOUND_WIZARD_HP_PER
                && currentHP < maxHP / 2) {
            setStrategy(new WizardOffensiveStrategy());
            return;
        }
        if (currentHP < maxHP / ST_LOW_BOUND_WIZARD_HP_PER
                && currentHP > ST_HIGH_BOUND_WIZARD_HP_PER) {
